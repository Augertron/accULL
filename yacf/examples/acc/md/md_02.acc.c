...
//#pragma acc kernels create(i, j, k) pcopy(f[NPARTS]) copy(pot,kin, nd,np) 
#pragma acc kernels create(i,j) pcopy(f[NPARTS], kin, pot) pcopyin(nd,np, pos[NPARTS], vel[NPARTS])
  for (i = 0; i < np; i++) {
    
    /* compute potential energy and forces */ // pcopyout(f[NPARTS]) pcopyin(nd, i)

     #pragma acc loop private(j)     
    for (j = 0; j < nd; j++)
      f[i][j] = 0.0;
    // pcopyin(pos[NPARTS], np, i, nd) pcopy(f[NPARTS], pot)
     #pragma acc loop reduction(+ : pot) private(j)  
    for (j = 0; j < np; j++) {
      vnd_t rij;
      double  d;
      int ii, k;
...
// pcopyin(vel[NPARTS],nd, np, i) copy(kin)
   /* compute kinetic energy */
   #pragma acc loop reduction(+ : kin) private(ii) 
   for (ii = 0; ii < nd; ii++) {
...
#pragma acc kernels loop create(i, j) copyin(rmass,dt)  pcopyin(f[NPARTS]) pcopy(vel[NPARTS],pos[NPARTS],a[NPARTS],_np,_nd) private(i, j) collapse(2)
  for (i = 0; i < _np; i++) 
    for (j = 0; j < _nd; j++) {
...
/*  OSCR_init (NUMTHREADS, "Molecular dynamic simulation", "Use md <Nparts> <Nsteps>", NUM_ARGS, 
	...
	NPARTS = DEFAULT_NPARTS; // OSCR_getarg_int(1);
  NSTEPS = DEFAULT_NSTEPS; // OSCR_getarg_int(2);
