...
#pragma acc kernels name("compute") create(i, j, k) pcopy(f[NPARTS]) copy(pot,kin) pcopyin(vel[NPARTS],pos[NPARTS]) copy(nd,np)
{
  i = 0;
...
void update(int np, int nd, vnd_t *pos, vnd_t *vel, vnd_t *f, vnd_t *a, double mass, double dt) {
 ...
 // collapse(2)
  #pragma acc kernels loop create(i, j) copyin(rmass,dt)  pcopyin(f[NPARTS]) pcopy(vel[NPARTS],pos[NPARTS],a[NPARTS],_np,_nd) private(i, j) name("update")  
for (i = 0; i < _np; i++) 
    for (j = 0; j < _nd; j++) {
